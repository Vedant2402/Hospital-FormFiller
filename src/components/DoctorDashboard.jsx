            ...
            <div>
              ...
              {doctor && (
                <p className="text-gray-600 text-sm">
                  Welcome, {doctor.name} ({doctor.email})
                </p>
              )}
            </div>
          </div>
            ...
            <div className="flex flex-wrap gap-2">
              {stats.byService.length === 0 ? (
                ...
                stats.byService.map((s) => (
                  <span
                    key={s.name}
                    className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary-50 text-primary-700 ring-1 ring-primary-200"
                  >
                    <ShieldCheck className="h-4 w-4" /> {s.name}{" "}
                    <span className="text-xs text-primary-800">{s.count}</span>
                  </span>
                ))
              )}
            </div>
        ...
        <div className="glass-card anim-fade-in">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-gray-900">
              Patient Submissions ({filtered.length})
            </h2>
          ...
          {filtered.length === 0 ? (
            <div className="text-center py-10">
              ...
              >
                Create your first Pre-Auth <ArrowRight className="h-4 w-4" />
              </Link>
            </div>
          ) : (
                ...
                <tbody>
                  {filtered.map((p) => (
                    <tr key={p.patientId} className="border-b">
                      ...
                      <td className="py-2 pr-4">
                        <span className="px-2 py-1 rounded-md bg-medical-50 text-medical-800">
                          {p.principalDiagnosisDescription || "—"}
                        </span>
                      </td>
